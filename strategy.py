        ...
        if day > 500:  # 1년 데이터 축적
            ...
            if generalcount != 0:
                try:
                    ...
                    print(self.market,"전체: ", self.generalcount,"오늘 전체: ", generalcount, "오늘 증가: ", particularcount, " total: ",
...
class UsebitcoinPPO:
    def __init__(self, market):
        ...
        self.indicators = Indicators()
        self.codelist = self.file.GetStockList(market)
        self.checktoday = CheckbitcoinToday()
        self.totalresult = TotalResult()
        self.codedaybasket = {}
        self.overlapppo = {}
        self.generalcount = 0
    ...
    def MackoverlapPPO(self, day, MAdayrange, stocklist, dayrange):  # 코스닥 500일기준 - 85.6 % (코스피 600일 83.7 % 유동성이 낮음)
        percent = 0
        generalcount = 0
        particularcount = 0
        if day > dayrange:  # 1년 데이터 축적
            for keys, val in self.codedaybasket.items():
                todaystockbasket = []
                for j in range(len(val)):
                    if j == day:
                        newval = [val[i] for i in range(j)]  # 다음날것을 미리 반영해서 확률에 넣어버림 그래서 뺌
                        if len(newval) >= MAdayrange and len(newval) > dayrange:
                            if len(newval) % dayrange != 0:
                                newval = [newval[-i] for i in range(1, dayrange+1)]
                                newval.reverse()  # 500일 데이터 기반으로 작동함. 500일씩 이동함
                            stockPPO = self.indicators.MakePPOFromFile(MAdayrange, newval)
                            stockoverlapppolist = self.totalresult.StockOverlapppoListFromFile(stockPPO, 0.5)
                            self.overlapppo[keys] = stockoverlapppolist
                            for i in range(0, MAdayrange):
                                todaystockbasket.append(val[j - MAdayrange + i + 1])
                            todayPPO = self.checktoday.MakePPOFromFile(MAdayrange, todaystockbasket)[0]
