...
def MakeTotalResult(PPO, increasepercent):
    result = []
    ppomethod = PPOMethod()
    ppomethod.Condition_Setting(PPO, increasepercent)
...
class StockParticularInfo:
    ...
    def GetTodayClosePrice(self):
        return self.todaycloseprice

    def GetTodayStartPrice(self):
        return self.todaystartprice

    def GetTodayHighPrice(self):
        return self.todayhighprice

    def GetTodayLowPrice(self):
        return self.todaylowprice

    def GetTodayPercent(self):
        return self.todaypercent
...
class PPOMethod:
    ...
    def Condition_Setting(self, PPO, increasepersent):
        self.DicInitialize(PPO)
        for i in range(round(self.PPOrange[-1] - self.PPOrange[0])):
            for keys, val in PPO.items():
                if self.PPOrange[0] + i <= val[0] < self.PPOrange[0] + i + 1 :
    ...
    def Condition_Setting_forMarket(self, PPO, increasepersent):
        self.DicInitialize_forMarket(PPO)
        iter = np.arange(0, self.PPOrange[-1] - self.PPOrange[0], 0.1)
        for i in iter:
            i = round(i, 1)
            for keys, val in PPO.items():
                if self.PPOrange[0] + i <= val[0] < self.PPOrange[0] + i + 0.1:
                    self.PPOrangecount[self.PPOrange[0] + i] += 1
                    self.daylist[self.PPOrange[0] + i] += [keys]
                    if val[2] <= increasepersent:
                        self.PPOrangePaticularcount[self.PPOrange[0] + i] += 1
    ...
    def Get_result_forMarket(self):
        ...
        for i in iter:
            i = round(i, 1)
            if self.PPOrangePaticularcount[self.PPOrange[0] + i] >= 5 and self.PPOrangePercent[self.PPOrange[0] + i] >= 85:
...
class CheckToday:
    ...
    def CheckTodayStockFromFile(self, middleresult, todayPPO):  # [이격도 , 오늘증감률, 고가증감률]
        for i in range(len(middleresult)):
            if middleresult[i] <= todayPPO[0] < middleresult[i] + 1:
                return 1
    ...
    def CheckTodayMarketFromFile(self, middleresult, todayPPO):  # [이격도 , 오늘증감률, 고가증감률]
        for i in range(len(middleresult)):
            if middleresult[i] <= todayPPO[0] < middleresult[i] + 0.1:
                return 1
        return 0
